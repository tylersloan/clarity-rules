...
type ConsAndDocs = Pick<IRule, 'documents' | 'conditions'>
type NoConsAndDocsOrUpdatedAt = Omit<
  IRule,
...
export function updateRule(rule: IRule): Promise<IRule> {
...
const transformRule = (
  ...
  const transformedRule = {
    ...
    documents: rule.actionIds.map((actionId) => {
...
const buildRule = async ({
  ...
  return Promise.resolve({
    ...
    actionIds: documents.map((d) => d.value),
